...
# ---------------------------
LINES_FILE = "Traveltimes_and_PublicTransport/hsl_lines.geojson"

lines = gpd.read_file(LINES_FILE)

lines["vehicle_ty_clean"] = (
    lines["vehicle_ty"]
    .astype(str)
    .str.strip()
...
# Keep only tram, metro, train and buses
lines = lines[lines["vehicle_ty_clean"].isin(["0", "1", "109", "3"])]

# Reproject to match map CRS
lines = lines.to_crs(epsg=3857)

# Clip transport lines to the grid extent
grid_area = gdf.unary_union
lines = gpd.clip(lines, grid_area)
...
for code, style in line_style.items():
    subset = lines[lines["vehicle_ty_clean"] == code]
...
# Title
#ax.set_title("Travel Time to Destination \nPublic Transport (Average, Rush hour)",fontsize=14)

ax.set_axis_off()

# Save
#plt.savefig("Pictures/Traveltimes/With_pt.png", dpi=600, bbox_inches="tight")
